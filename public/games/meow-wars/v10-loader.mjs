/* Meow Wars v1.0 production loader.
 * Real v0.5 gameplay payload + cumulative HD/game-feel/polish/presentation/release layers.
 */
const VERSION = '1.0.0';
const BUILD = 'mw-v10-release-20260918a';

async function readProductionSource() {
  const urls = Array.from({ length: 6 }, (_, i) => './v05-payload-' + (i + 1) + '.txt?v=10a');
...
async function start() {
  const [v05, hd, gamefeel, polish, presentation, release] = await Promise.all([
    readProductionSource(),
    readLayer('./v06-hd.js?v=10a', 'v0.6 HD layer'),
    readLayer('./v07-gamefeel.js?v=10a', 'v0.7.1 game-feel layer'),
    readLayer('./v08-battle-polish.js?v=10a', 'v0.8 battle-polish layer'),
    readLayer('./v09-battle-presentation.js?v=10a', 'v0.9 battle-presentation layer'),
    readLayer('./v10-release.js?v=10a', 'v1.0 release layer')
  ]);

  const source = composeSource(v05, hd, gamefeel, polish, presentation, release);
