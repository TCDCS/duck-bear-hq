...
const MW_BUILD = 'MW-HD-20260918-01';
...
const skinByArena = {
  ...
  'oconnell-bridge': 'dublin-stone',
  'westminster-bridge': 'london-stone',
...
const drawers = {
  ...
  'taj-mahal': drawTaj,
  'oconnell-bridge': drawOconnell,
  'westminster-bridge': drawWestminster,
...
function ensureBackgroundTextures(scene, arenaId) {
  for (let layer = 0; layer < 3; layer += 1) {
    ...
    const texture = scene.textures.createCanvas(key, MW_VIEW_W, MW_VIEW_H);
    const c = texture.getContext();
    c.clearRect(0, 0, MW_VIEW_W, MW_VIEW_H);
    c.imageSmoothingEnabled = true;
    c.imageSmoothingQuality = 'high';
    c.drawImage(source, 0, 0, MW_SOURCE_W, MW_SOURCE_H, 0, 0, MW_VIEW_W, MW_VIEW_H);
    texture.refresh();
    source.width = 1;
    source.height = 1;
  }
