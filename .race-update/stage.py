...
with zipfile.ZipFile('verification/reviewed-setup-source.zip','w',zipfile.ZIP_DEFLATED) as archive:
 for path in files:archive.write(path,path)
 archive.write('docs/verification/setup-seven.json','docs/verification/setup-seven.json')
shutil.rmtree('.race-update')
subprocess.check_call(['git','config','user.name','github-actions[bot]'])
subprocess.check_call(['git','config','user.email','41898282+github-actions[bot]@users.noreply.github.com'])
subprocess.check_call(['git','add','--',*files.keys(),'docs/verification/setup-seven.json','.race-update'])
