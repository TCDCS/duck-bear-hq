...
def game(page):page.goto(BASE+'/games/mango-mayhem/?verify=1',wait_until='networkidle');page.wait_for_function('window.MangoMayhem')
...
def snapshot(page):return page.evaluate('window.MangoMayhem.inspect()')
with sync_playwright() as p:
 ...
 try:
  ...
  pa.wait_for_function("document.getElementById('saveStatus').textContent==='Cloud saved'")
  ...
  check('device B collected mangos in the actual game',len(snapshot(pc)['progress']['levels']['dublin']['mangoIds'])>0)
  ...
  check('device A receives device B progress',len(snapshot(pa)['progress']['levels']['dublin']['mangoIds'])>0)
  # Pause and local save stay usable during a real network outage.
  ...
  pc.keyboard.down('ArrowRight');pc.wait_for_timeout(1900);pc.keyboard.up('ArrowRight');pc.keyboard.press('Escape');before=len(snapshot(pc)['progress']['levels']['dublin']['mangoIds'])
  ...
  check('offline queue reached the other browser',len(snapshot(pa)['progress']['levels']['dublin']['mangoIds'])>=before)
  pa.screenshot(path=str(OUT/'two-device-profile.png'))
  check('password fields are not stored in local saves',all('password' not in v.lower() for v in pa.evaluate('Object.values(localStorage)')))
