...
with sync_playwright() as pw:
    ...
    try:
        ...
        check('local setup exposes independent match toggles',all(page.locator(f'#{id}').is_visible() for id in ['health-damage','visible-bruising','arena-hazards','friendly-fire']))
        ...
        hold_key(page,'KeyF',160)
        check('Player 1 can deliberately throw a held weapon',page.locator('#p1-weapon').inner_text()=='FISTS',page.locator('#p1-weapon').inner_text())
        hold_key(page,'KeyE',160)
        check('jump input keeps the live match healthy',not page.locator('#fatal').is_visible())
        ...
        page.locator('#arena-select').select_option('wrestling-arena')
        page.locator('#mode-select').select_option('mode-one-v-one')
        page.locator('#health-damage').uncheck()
        page.locator('#arena-hazards').uncheck()
        page.get_by_role('button',name='START FIGHT').click()
        wait_match_ready(page,'damage-off local')
        page.locator('#game-canvas').focus();page.keyboard.down('KeyD');page.keyboard.down('Space');page.wait_for_timeout(2600);page.keyboard.up('Space');page.keyboard.up('KeyD');page.wait_for_timeout(250)
        check('damage-off keeps HP unchanged while the match keeps simulating',page.locator('#p1-hp').inner_text()=='100 HP' and page.locator('#p2-hp').inner_text()=='100 HP',{'p1':page.locator('#p1-hp').inner_text(),'p2':page.locator('#p2-hp').inner_text()})
        check('damage-off does not trigger a fatal browser state',not page.locator('#fatal').is_visible())
        page.get_by_role('button',name='RETURN TO MENU').click()

        page.get_by_role('button',name='LOCAL PLAY').click()
        page.locator('#health-damage').check();page.locator('#arena-hazards').check()
        page.locator('#arena-select').select_option('wrestling-arena')
        page.locator('#character-select').select_option('character-hero')
        page.locator('#mode-select').select_option('mode-king-of-ring')
        page.get_by_role('button',name='START FIGHT').click()
        wait_match_ready(page,'King of the Ring local')
        page.wait_for_function("""() => document.querySelector('#objective-status') && !document.querySelector('#objective-status').hidden""")
        objective=page.locator('#objective-status').inner_text()
        check('King of the Ring runs as a live objective mode','KING OF THE RING' in objective and '30' in objective,objective)
        check('four-fighter objective mode remains live',not page.locator('#round-over').is_visible() and not page.locator('#fatal').is_visible())
        page.screenshot(path=str(OUT/'king-of-the-ring.png'))
        page.get_by_role('button',name='RETURN TO MENU').click()
        check('objective mode returns to the native menu',page.locator('#main-menu').is_visible())
        ...
        check('host can configure the room after creation',page.locator('#online-arena').is_enabled() and page.locator('#online-mode').is_enabled() and page.locator('#online-arena-hazards').is_enabled())
        page.locator('#online-arena').select_option('WrestlingArena')
        page.locator('#online-mode').select_option('OneVsOne')
        page.locator('#online-arena-hazards').uncheck()
        ...
        check('online settings propagate to clients read-only',not guest.locator('#online-arena-hazards').is_checked() and not guest.locator('#online-arena-hazards').is_enabled())
        ...
        guest.locator('#game-canvas').focus();hold_key(guest,'KeyD',700);hold_key(guest,'KeyE',120);page.wait_for_timeout(500)
        check('online input and snapshot loop stays live after client movement and jump',not page.locator('#fatal').is_visible() and not guest.locator('#fatal').is_visible())
