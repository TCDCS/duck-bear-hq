...
const {
 MODE_RULES,OBJECTIVE_RULES,createModeState,resolveElimination,
...
test('browser mode rules preserve all eight Danao launch modes',()=>{
 assert.deepEqual(Object.keys(MODE_RULES),['OneVsOne','TwoVsTwo','FreeForAll','RoyalRumble','MangoGrab','HotBomb','KingOfTheRing','Heist']);
 for(const kind of ['OneVsOne','TwoVsTwo','FreeForAll','RoyalRumble'])assert.equal(MODE_RULES[kind].elimination,true,kind);
 for(const kind of ['MangoGrab','HotBomb','KingOfTheRing','Heist']){
...
test('King of the Ring only counts uncontested centre time',()=>{
 const state=createModeState('KingOfTheRing',[0,1]);
