...
test('Meow Wars v0.7 composes and compiles against the real v0.5 production payload', () => {
  const source = composeV07();
  assert.doesNotThrow(() => new Function(source));
  assert.match(source, /mw-v07-gamefeel-20260918a/);
...
test('Meow Wars v0.7 replaces the Dublin scene with Ha\u2019penny Bridge and richer streetscape cues', () => {
  ...
  assert.match(source, /HA'PENNY BRIDGE INN/);
  assert.match(source, /MERCHANT'S ARCH/);
  assert.match(source, /drawSpire\(c, 1006, 405, 385\)/);
});

test('Meow Wars v0.7 adds weapon, explosion, cat-reaction and battlefield ambience polish', () => {
...
test('Meow Wars v0.7 shell and loader expose current version/build and retain the 4K HD layer', () => {
  ...
  assert.match(html, /v07-loader\.mjs\?v=7a/);
  assert.match(html, /data-version="0\.7\.0"/);
  assert.match(html, /data-build="mw-v07-gamefeel-20260918a"/);
  assert.match(loader, /v06-hd\.js\?v=7a/);
  assert.match(loader, /v07-gamefeel\.js\?v=7a/);
