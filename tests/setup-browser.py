"""Native setup/UI checks with sampled rendered frames, not a device frame-rate benchmark."""
...
async def main():
 async with async_playwright() as p:
  ...
  try:
   ...
   # CI uses a CPU-backed GPU. Sample actual draws without building a GPU queue;
   # leave the real animation clock, countdown, input and simulation untouched.
   await page.evaluate('()=>{const renderer=WackyRaces.renderer;window.savedDraw=renderer.render;window.raceRendered=false;renderer.render=function(r,dt,options){if(options.menu)return savedDraw.call(this,r,dt,options);if(!window.raceRendered){savedDraw.call(this,r,dt,options);this.gl?.finish();window.raceRendered=true;}}}')
   await page.click('#startRace');await wait(page,'WackyRaces.screen==="race"&&WackyRaces.race.countdown===0')
   await check('selected circuit renders a real racing frame',await page.evaluate('window.raceRendered===true'))
   ...
   await page.evaluate('()=>{savedDraw.call(WackyRaces.renderer,WackyRaces.race,1/60,{menu:false,reduced:true});WackyRaces.renderer.gl?.finish()}')
   await page.screenshot(path=str(OUT/'seven-racers-standings.png'),timeout=90000)
   ...
   for width,height,label in [(390,844,'portrait'),(844,390,'landscape')]:
    await page.set_viewport_size({'width':width,'height':height});await page.evaluate('()=>{savedDraw.call(WackyRaces.renderer,WackyRaces.race,1/60,{menu:false,reduced:true});WackyRaces.renderer.gl?.finish()}')
    ...
    await page.screenshot(path=str(OUT/('standings-'+label+'.png')),timeout=90000)
   await page.close()
...
if __name__=='__main__':
 report={'base':BASE,'scope':'native menu, selected options, live standings and seven-car grid; sampled rendering, not a frame-rate benchmark','passed':False}
