...
test('browser create and join mutations reject a mismatched origin',async()=>{assert.equal((await routeDanaoMultiplayer(request('/api/danao/create','POST',{name:'x'},'https://evil.test'),{})).status,403);});
test('native create requests without an Origin header are allowed through normal validation',async()=>{const r=await routeDanaoMultiplayer(request('/api/danao/create','POST',{name:'x'},null),{});assert.equal(r.status,503);});
