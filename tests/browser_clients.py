...
async def load(page,path):
 ...
 if path.startswith('/games'):
  await page.wait_for_function('!!window.WackyRaces',timeout=60000)
...
async def main():
 async with async_playwright() as p:
  ...
  try:
   ...
   await host.fill('#friendName','Zachary');await host.select_option('#friendAvatar','0');await host.select_option('#createCircuit','5');await host.click('#roomCreate');await host.wait_for_function('WackyRaces.network.connected&&WackyRaces.network.room.players[0].connected',timeout=30000)
   ...
   await guest.select_option('#friendAvatar','1');await guest.fill('#friendName','Guannan');await guest.select_option('#friendVehicle','bus');await guest.click('#roomJoin');await guest.wait_for_function('WackyRaces.network.connected&&WackyRaces.network.room.players.length===2',timeout=30000)
   await host.wait_for_function('WackyRaces.network.room.players.length===2')
   await check('host waits until friend is ready',await host.locator('#roomStart').is_disabled())
   await guest.click('#roomReady');await host.wait_for_function('!document.getElementById("roomStart").disabled')
   ...
   await host.click('#roomStart');await host.wait_for_function('WackyRaces.screen==="race"&&WackyRaces.race.countdown===0',timeout=30000);await guest.wait_for_function('WackyRaces.screen==="race"&&WackyRaces.race.countdown===0',timeout=30000)
   ...
   await host.wait_for_timeout(2500);await host.screenshot(path=str(OUT/'online-race.png'))
   ...
   # Render validation above is separate from the time-sensitive transport checks below.
   ...
   await host.wait_for_function('WackyRaces.race.racers[0].steer>.8&&WackyRaces.race.racers[0].vx>1',timeout=15000)
   ...
   await guest.keyboard.down('ArrowLeft');await guest.wait_for_function('WackyRaces.race.racers[0].steer<-.8&&WackyRaces.race.racers[0].vx<0',timeout=15000);await guest.keyboard.up('ArrowLeft')
   ...
   own=(await state(host))['id'];await host.evaluate('WackyRaces.network.socket.close()');await host.wait_for_function('WackyRaces.network.attempt>0||!WackyRaces.network.connected',timeout=10000);await host.wait_for_function('WackyRaces.network.connected',timeout=30000)
   ...
   await host.wait_for_function('document.getElementById("resultsDialog").open',timeout=210000);await guest.wait_for_function('document.getElementById("resultsDialog").open',timeout=210000)
   ...
   await guest.click('#resultNext');await host.wait_for_function('WackyRaces.network.room.phase==="lobby"');await check('host can bring everyone back for another race',await host.locator('#friendsDialog').evaluate('(el)=>el.open'))
   ...
   await host.click('#roomLeave');await host.bring_to_front();await host.click('#startRace');await host.wait_for_timeout(300);print('SOLO STATE',await host.evaluate('({hidden:document.hidden,focus:document.hasFocus(),phase:WackyRaces.race.phase,countdown:WackyRaces.race.countdown,active:WackyRaces.network.active})'),flush=True);await host.wait_for_function('WackyRaces.race.countdown===0');await check('solo still starts after an online session',await host.evaluate('!WackyRaces.race.multiplayer'))
