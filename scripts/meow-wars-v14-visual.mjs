...
const outputDir = new URL('../artifacts/meow-wars-v14/', import.meta.url);
...
try {
  ...
  for (let arenaIndex = 0; arenaIndex < arenaCount; arenaIndex += 1) {
    ...
    if (variant === 'after') {
      ...
      await page.waitForFunction(() => globalThis.__MEOW_WARS_V13_STATS?.ambientFrames > 2, { timeout: 4000 });
      await page.waitForFunction(() => globalThis.__MEOW_WARS_V14_STATS?.scenesLit > 0, { timeout: 4000 });
      await page.waitForFunction(() => globalThis.__MEOW_WARS_V14_STATS?.lightingFrames > 2, { timeout: 4000 });
    }
    ...
    if (variant === 'after' && [3,4,5,6].includes(arenaIndex)) {
      ...
      await page.screenshot({
        path: output('after-env-' + String(arenaIndex + 1).padStart(2, '0') + '-' + name + '.png'),
        fullPage: true
      });
    }
    ...
    if (variant === 'after' && arenaIndex === 0) {
      ...
      // Glass material pass: shatter the greenhouse rather than relying only on metal debris.
      await page.evaluate(() => {
        const scene = globalThis.__MEOW_WARS_GAME_SCENE;
        const prop = scene.__mw11Props.find((p) => p.type === 'greenhouse' && !p.destroyed);
        if (!prop) return;
        scene.explode(prop.x, prop.y - 34, {
          id:'dynamite', family:'conventional', damage:150, blastRadius:92, craterScale:1.08
        }, 'blue-1');
      });
      await page.waitForTimeout(120);
      await page.screenshot({ path: output('after-01c-glass-material.png'), fullPage: true });
    }
  }
  ...
  if (variant === 'after') {
    ...
    if (transition.build !== 'mw-v14-atmosphere-materials-20260918a') {
      throw new Error('Unexpected v1.4 build: ' + transition.build);
    }
  }
