...
const code=['src/index.js','src/worker-games.js','src/game-routes.js','public/app.js','public/home.js','public/account-navigation.js','public/kart-games.js','public/sw.js',...readdirSync(resolve(ROOT,'src/multiplayer')).filter(f=>f.endsWith('.mjs')).map(f=>'src/multiplayer/'+f)];
...
try{const db=new DatabaseSync(':memory:');db.exec(readFileSync(resolve(ROOT,'migrations/0001_schema.sql'),'utf8'));db.exec(readFileSync(resolve(ROOT,'migrations/0002_seed.sql'),'utf8'));if(db.prepare('SELECT COUNT(*) AS n FROM products').get().n<10||db.prepare('SELECT COUNT(*) AS n FROM rewards').get().n<5)failures.push('Seed data count is unexpectedly low.');db.close();}catch(e){failures.push('SQLite migration check failed: '+e.message);}
