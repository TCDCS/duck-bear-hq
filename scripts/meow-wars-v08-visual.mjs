...
const outputDir = new URL('../artifacts/meow-wars-v08/', import.meta.url);
...
try {
  ...
  for (let arenaIndex = 0; arenaIndex < arenaCount; arenaIndex += 1) {
    ...
    if (variant === 'after' && arenaIndex === 0) {
      ...
      await page.waitForFunction(() => globalThis.__MEOW_WARS_V08_STATS?.weaponAccent > 0, { timeout: 4000 });
      await page.waitForFunction(() => globalThis.__MEOW_WARS_V08_STATS?.projectileTrail > 0, { timeout: 5000 });
      await page.waitForFunction(() => globalThis.__MEOW_WARS_V08_STATS?.terrainDebris > 0, { timeout: 9000 });
      await page.waitForFunction(() => globalThis.__MEOW_WARS_V08_STATS?.arenaImpact > 0, { timeout: 9000 });
      await page.waitForTimeout(90);
      await page.screenshot({ path: output('after-01b-v08-impact.png'), fullPage: true });

      const stats = await page.evaluate(() => globalThis.__MEOW_WARS_V08_STATS);
      if (!stats || stats.weaponAccent < 1 || stats.projectileTrail < 1 || stats.terrainDebris < 1 || stats.arenaImpact < 1 || stats.hudRefresh < 1) {
        throw new Error('v0.8 polish counters did not activate: ' + JSON.stringify(stats));
      }
    }
  }
