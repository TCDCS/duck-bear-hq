...
const outputDir = new URL('../artifacts/meow-wars-v16-online/', import.meta.url);
...
async function waitGame(page) {
  ...
  await page.waitForFunction(() =>
    globalThis.__MEOW_WARS_BUILD === 'mw-v16-menu-city-life-20260918a',
    null,
    { timeout: 30000 }
  );
...
try {
  ...
  // v1.6 moves ONLINE 1V1 into the canvas mode row and deliberately hides the old floating launcher.
  await Promise.all([waitGame(host), waitGame(guest)]);
  await Promise.all([
    host.waitForFunction(() =>
      globalThis.__MEOW_WARS_V16_STATS?.menusCleaned > 0 &&
      getComputedStyle(document.getElementById('mw-online-launcher')).display === 'none',
      null,
      { timeout: 15000 }
    ),
  ...
  // Host chooses O'Connell Bridge / Spire before creating the room.
  for (let i = 0; i < 4; i += 1) {
    await logicalClick(host, 955, 257);
    await host.waitForTimeout(80);
  }

  await logicalClick(host, 915, 167);
  ...
  await logicalClick(guest, 915, 167);
  ...
  if (finalState.some((state) => state.build !== 'mw-v16-menu-city-life-20260918a')) {
